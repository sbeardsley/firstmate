...
FM_LOOP_BLOCK=1   # set for crewmate panes by bin/fm-spawn.sh; lets the loop-detector extension (pi-extensions/firstmate-pi-loop.ts) best-effort abort a stuck turn or block the looping tool call after notifying (notification-only when unset). The primary firstmate session never sets it.
