...
FM_LOOP_BLOCK=1   # set for crewmate panes by bin/fm-spawn.sh; lets the loop-detector extension (pi-extensions/firstmate-pi-loop.ts) best-effort abort a stuck reasoning turn. The primary firstmate session never sets it.
