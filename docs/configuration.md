...
Per-dispatch exclusions are always logged to stderr, while the equivalent static config facts are reported only under `FM_BOOTSTRAP_VERBOSE_FACTS=1` as `BOOTSTRAP_INFO:` lines, so an inherited secondmate home missing credentials never becomes a recurring startup diagnostic.
See [`docs/examples/crew-dispatch.json`](examples/crew-dispatch.json) for a starting point to copy into local `config/crew-dispatch.json`.
When the file exists, bootstrap validates it with `jq`.
Valid files stay silent by default; with `FM_BOOTSTRAP_VERBOSE_FACTS=1`, bootstrap emits `BOOTSTRAP_INFO: crew dispatch active config/crew-dispatch.json`, one `BOOTSTRAP_INFO:` fact per rule and default profile, and the quota-vendor facts described above for `quota-balanced` candidates.
...
FM_CAPTAIN_RE='done:|needs-decision:|blocked:|failed:|PR ready|checks green|ready in branch|merged|loop:'   # status regex that makes watcher and daemon signal/stale/scan output captain-relevant
FM_LOOP_BLOCK=1   # set for crewmate panes by bin/fm-spawn.sh; lets the loop-detector extension (pi-extensions/firstmate-pi-loop.ts) best-effort abort a stuck turn or block the looping tool call after notifying (when unset, notification-only apart from the FM_LOOP_BAN session ban). The primary firstmate session never sets it.
FM_LOOP_BAN=1   # set to 0 to disable the loop-detector extension's session-scoped ban of tool calls that triggered a sequence loop; defaults to on and blocks banned calls independently of FM_LOOP_BLOCK. Detector kinds and thresholds live in the pi-extensions/firstmate-pi-loop.ts header.
