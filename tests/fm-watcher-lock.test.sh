...
test_watch_restart_attaches_to_healthy_peer() {
  ...
  mark_pr_check_migration_complete "$state"
  # The peer stands in for a watcher that outlives the restart's TERM. Node
  # installs its SIGTERM handler asynchronously, so it must announce readiness
  # before the arm runs: an unsynchronized peer is still killable by that TERM,
  # which leaves a dead-pid lock for the fresh child to steal and turns the
  # honest "attached" report into an equally honest but unintended "started".
  ...
  [ -s "$ready" ] || fail "TERM-resistant peer never signalled readiness"
